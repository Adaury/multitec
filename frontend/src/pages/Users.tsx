...
export function Users() {
  ...
  if (!isAdmin) {
    return (
      <div className="py-4">
        <Card>
          <p className="text-sm text-gray-600">Solo un administrador puede gestionar usuarios.</p>
        </Card>
      </div>
    )
  }
            ...
            <Card key={u.id} className="space-y-2">
              ...
              {isEditing && (
                <div className="space-y-2 border-t border-gray-100 pt-2">
                  <Field label="Rol">
                    <select
                      className="w-full rounded-xl border border-gray-200 bg-white px-4 py-3 text-base disabled:opacity-50"
                      value={u.role}
                      disabled={isSelf}
